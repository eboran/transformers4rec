...
def delete_some_features(schema, cat, cont, time):
    if not cat:
...
def run():
    for max_sequence_length in L:
        SCHEMA_PATH = f"model_configs_and_results/length/{max_sequence_length}/schema_demo.pb"

        sys.stdout = open(f'model_configs_and_results/length/{max_sequence_length}/prints_withF.txt', 'wt')

        print("max_sequence_length:", max_sequence_length)

        schema = Schema().from_proto_text(SCHEMA_PATH)

        schema = delete_some_features(schema, True, True, True)
