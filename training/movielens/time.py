...
def run():
    for max_sequence_length in L:
        for time in ["day", "week", "year", "sin"]:

            config = dict()
            config[time] = True

            SCHEMA_PATH = f"model_configs_and_results/time/{max_sequence_length}/schema_demo.pb"

            sys.stdout = open(f'model_configs_and_results/time/{max_sequence_length}/print_{time}.txt', 'wt')
